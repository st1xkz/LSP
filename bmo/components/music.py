...
async def _join_voice(
    ...
    if ctx.client.cache and ctx.client.shards:
        ...
        await ctx.client.shards.update_voice_state(
            ctx.guild_id, channel_id, self_deaf=True
        )
...
@tanjun.with_str_slash_option(
    "song", "The title or youtube link of the song you want to play."
)
...
async def _play_track(
    ctx: tanjun.abc.Context, song: str, lavalink: lavasnek_rs.Lavalink
) -> None:
...
@tanjun.as_slash_command(
    "stop", "Stops the currently playing song, skip to play again."
)
